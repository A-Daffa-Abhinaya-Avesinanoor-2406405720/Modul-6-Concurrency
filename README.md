...
Pada milestone ini, server diubah menjadi multithreaded dengan memakai `ThreadPool` agar request tidak lagi selalu diproses satu per satu oleh main thread. Di `main`, setelah `TcpListener` dibuat, program membuat `ThreadPool::new(4)` lalu setiap connection dari `listener.incoming()` tidak langsung diproses dengan `handle_connection(stream)`, tetapi dikirim ke `pool.execute(|| { handle_connection(stream); })`. Dengan begitu, request yang masuk digabung sebagai tugas dan dapat dikerjakan oleh salah satu worker thread yang tersedia di dalam pool.

Cara kerja `ThreadPool` pada `src/lib.rs`, `ThreadPool::new(size)` membuat channel `mpsc` untuk mengirim job, lalu membuat worker sesuai ukuran pool. Setiap `Worker` menjalankan thread sendiri dan berbagi akses ke `Receiver` melalui `Arc<Mutex<_>>`, karena receiver yang sama perlu dipakai oleh banyak thread secara aman. Method `execute` menerima closure, digabung menjadi `Job = Box<dyn FnOnce() + Send + 'static>`, lalu kirim ke channel. Salah satu worker yang sedang idle akan mengambil job dari queue, print pesan bahwa mendapat tugas, lalu execute closure tersebut. Setelah selesai, worker kembali menunggu job berikutnya. Dengan ini, beberapa request bisa diproses secara concurrent sampai batas jumlah thread yang tersedia.

Hal yang saya pelajari dari milestone ini adalah bahwa multithreading tidak sekadar “menambah thread”, tetapi juga perlu desain pembagian kerja yang terkontrol. Saya jadi paham mengapa thread pool lebih baik daripada membuat thread baru tanpa batas untuk setiap request, karena pool membuat jumlah thread tetap terbatas dan lebih aman terhadap pemborosan resource. Selain itu, saya juga belajar penggunaan `mpsc` channel untuk mendistribusikan tugas, `Arc<Mutex<T>>` untuk berbagi data antar-thread dengan aman. Closure pada job perlu memiliki bound `FnOnce + Send + 'static` agar bisa dipindahkan dan dijalankan oleh worker thread.
</details>

<details>
<summary> Commit Bonus: </summary>

Pada bonus ini, saya mengganti constructor `ThreadPool` dari `new` menjadi `build` pada `src/lib.rs`. Perubahan utamanya pada cara menangani error. Jika sebelumnya `new(size)` langsung return `ThreadPool` dan akan `panic` saat size pool bernilai `0`, sekarang `build(size)` return `Result<ThreadPool, &'static str>`. Dengan begitu, validasi size thread pool dilakukan lebih eksplisit dan caller bisa memutuskan sendiri bagaimana menangani kondisi error tersebut.

Di `main`, perubahan ini terlihat pada pemanggilan `ThreadPool::build(4).expect("Failed to build thread pool")`. Constructor yang berpotensi gagal return `Result`, tidak langsung menghentikan program dari dalam function. Dibandingkan `new`, `build` lebih fleksibel karena dapat dipakai pada konteks yang membutuhkan error handling yang lebih rapi, misalnya jika nanti ukuran thread pool berasal dari input user atau file konfigurasi.

Hal yang saya pelajari dari bonus ini adalah pentingnya membedakan antara API yang infallible dan fallible. `new` cocok dipakai jika pembuatan object hampir selalu berhasil, sedangkan `build` lebih tepat jika ada kemungkinan gagal dan kita ingin memberi kesempatan kepada caller untuk menanganinya.
</details>
